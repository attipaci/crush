...
if exist windows\nul  (
  echo . Copying Windows batch files to main CRUSH directory...
  attrib -h windows\*.bat
  copy /y /l windows\* . >nul 2>&1
  del /f /q install.bat >nul 2>&1
)

:: Now delete all the bash scripts
...
:: Hide 'wrapper.bat' and 'autoconf.bat'...
echo . Hiding 'autoconf.bat' and 'wrapper.bat'
attrib +h wrapper.bat
attrib +h autoconf.bat
...
:: Removing windows\ subdirectory from installation
echo . Removing windows\ distribution subdirectory from installation
del /s /f /q "%CRUSH%\windows" >nul 2>&1
rmdir /s /q "%CRUSH%\windows" >nul 2>&1
